...
export default function AddProduct() {
  ...
  const handleSubmit = (e) => {
    ...
    toast.custom((t) => (
      <div className={`${t.visible ? 'animate-enter' : 'animate-leave'} flex items-start gap-4 p-4 rounded-lg bg-white dark:bg-[#1a2632] shadow-lg border border-green-100 dark:border-green-900 border-l-4 border-l-green-500 max-w-sm w-full`}>
        <div className="text-green-500 mt-0.5">
          <CheckCircle size={20} />
        </div>
        <div className="flex flex-col gap-1">
          <h4 className="text-sm font-bold text-[#101418] dark:text-white leading-none">Success</h4>
          <p className="text-sm text-[#5e758d] dark:text-[#9ba8b8] leading-tight">Product successfully listed!</p>
        </div>
        <button onClick={() => toast.dismiss(t.id)} className="text-[#5e758d] hover:text-[#101418] dark:text-[#9ba8b8] dark:hover:text-white ml-auto">
          <X size={18} />
        </button>
      </div>
    ));
      ...
      <main className="flex flex-col items-center justify-start flex-1 py-10 px-4 md:px-0 relative">
        ...
        <div className="w-full max-w-[600px] bg-white dark:bg-[#1e2730] rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 md:p-10 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-2 text-center md:text-left">
            <h1 className="text-[#101418] dark:text-white tracking-tight text-3xl font-bold leading-tight">List a New Product</h1>
            <p className="text-[#5e758d] dark:text-[#9ba8b8] text-sm font-normal leading-normal">Fill in the details below to add your item to the marketplace.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              <label className="text-[#101418] dark:text-white text-base font-medium leading-normal">Product Images</label>
              <div 
                ...
                {imagePreview ? (
                  ...
                  <>
                    ...
                    <div className="flex flex-col items-center gap-1 text-center relative z-10">
                      <p className="text-[#101418] dark:text-white text-base font-semibold">Click to upload or drag and drop</p>
                      <p className="text-[#5e758d] dark:text-[#9ba8b8] text-sm">SVG, PNG, JPG or GIF (max. 800x400px)</p>
                    </div>
                  </>
                )}
                <input 
            ...
            <div className="flex flex-col gap-6">
              <div className="flex flex-col gap-2">
                <label className="text-[#101418] dark:text-white text-sm font-medium leading-normal" htmlFor="product-name">Product Name</label>
                <input 
                  id="product-name"
                  type="text"
                  value={name}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  <label className="text-[#101418] dark:text-white text-sm font-medium leading-normal" htmlFor="price">Price</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <span className="text-[#5e758d] dark:text-[#9ba8b8] sm:text-sm">$</span>
                    </div>
                    <input 
                      id="price" 
                      type="number" 
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[#101418] dark:text-white text-sm font-medium leading-normal" htmlFor="category">Category</label>
                  <div className="relative">
                    <select 
                      id="category"
                      value={category}
